...
abstract class BasesfGuardUserFormFilter extends BaseFormFilterDoctrine
{
  ...
  public function addAssignaturaListColumnQuery(Doctrine_Query $query, $field, $values)
  {
    if (!is_array($values))
    {
      $values = array($values);
    ...
    $query
      ...
      ->andWhereIn('UsuariTeAssignatura.assignatura_id', $values)
    ;
  }

  public function addUsuarisEspiatsListColumnQuery(Doctrine_Query $query, $field, $values)
  {
    if (!is_array($values))
    {
      $values = array($values);
    ...
    $query
      ->leftJoin($query->getRootAlias().'.UsuariEspiaUsuari UsuariEspiaUsuari')
      ->andWhereIn('UsuariEspiaUsuari.usuari_objectiu', $values)
    ;
  }
  ...
  public function addSfGuardUserListColumnQuery(Doctrine_Query $query, $field, $values)
  {
    ...
    $query
      ->leftJoin($query->getRootAlias().'.UsuariEspiaUsuari UsuariEspiaUsuari')
      ->andWhereIn('UsuariEspiaUsuari.usuari_base', $values)
    ;
  }
  ...
  public function addApplicationsListColumnQuery(Doctrine_Query $query, $field, $values)
  {
    ...
    $query
      ...
      ->andWhereIn('sfOauthServerDeveloper.consumer_id', $values)
    ;
  }
  ...
  public function getFields()
  {
    return array(
      ...
      'assignatura_list'     => 'ManyKey',
      'usuaris_espiats_list' => 'ManyKey',
